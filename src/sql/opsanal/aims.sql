-- AIMS (Awards Information Management System) Data Processing
-- Data has already been parsed into columns by nara.py using schema field positions
-- Date range: 1988-2004 (military personnel awards and commendations)
...
SELECT
    ...
    CASE 
        ...
        THEN 
            CASE 
                WHEN SUBSTR(TRIM(SECBD_MTG), -4) BETWEEN '1988' AND '2004'
    ...
    CASE 
        ...
        THEN 
            CASE 
                WHEN SUBSTR(TRIM(MERIT_START), -4) BETWEEN '1988' AND '2004'
    ...
    CASE 
        ...
        THEN 
            CASE 
                WHEN SUBSTR(TRIM(MERIT_END), -4) BETWEEN '1988' AND '2004'
    ...
    CASE 
        ...
        THEN 
            CASE 
                WHEN SUBSTR(TRIM(ORIG_RECMD_DATE), -4) BETWEEN '1988' AND '2004'
    ...
    CASE 
        ...
        THEN 
            CASE 
                WHEN SUBSTR(TRIM(ENDRSMT_DATE), -4) BETWEEN '1988' AND '2004'
    ...
    CASE 
        ...
        THEN 
            CASE 
                WHEN SUBSTR(TRIM(APPR_AWD_DATE), -4) BETWEEN '1988' AND '2004'
    ...
    CASE 
        ...
        THEN 
            CASE 
                WHEN SUBSTR(TRIM(MAIL_DATE), -4) BETWEEN '1988' AND '2004'
    ...
    CASE 
        ...
        THEN 
            CASE 
                WHEN SUBSTR(TRIM(CMC_TO_MHM), -4) BETWEEN '1988' AND '2004'
...
LEFT JOIN aims_lookup_camp AS camp_lookup ON 
    TRIM(CAMPAIGN) = camp_lookup.code AND CAMPAIGN IS NOT NULL AND CAMPAIGN != 'nan' AND CAMPAIGN != ''
LEFT JOIN aims_lookup_grd AS grd_lookup ON 
    TRIM(GRADE_RATING) = grd_lookup.code AND GRADE_RATING IS NOT NULL AND GRADE_RATING != 'nan' AND GRADE_RATING != ''
LEFT JOIN aims_lookup_corp AS corp_lookup ON 
    TRIM(CORPS_MEM) = corp_lookup.code AND CORPS_MEM IS NOT NULL AND CORPS_MEM != 'nan' AND CORPS_MEM != ''
LEFT JOIN aims_lookup_awd AS awd_lookup_recmd ON 
    TRIM(RECMD_AWD) = awd_lookup_recmd.code AND RECMD_AWD IS NOT NULL AND RECMD_AWD != 'nan' AND RECMD_AWD != ''
LEFT JOIN aims_lookup_act AS act_lookup ON 
    TRIM(ACTION_AREA) = act_lookup.code AND ACTION_AREA IS NOT NULL AND ACTION_AREA != 'nan' AND ACTION_AREA != ''
LEFT JOIN aims_lookup_awd AS awd_lookup_sec ON 
    TRIM(SEC_ENDRSR_RECMD) = awd_lookup_sec.code AND SEC_ENDRSR_RECMD IS NOT NULL AND SEC_ENDRSR_RECMD != 'nan' AND SEC_ENDRSR_RECMD != ''
LEFT JOIN aims_lookup_awd AS awd_lookup_appr ON 
    TRIM(APPR_AWD) = awd_lookup_appr.code AND APPR_AWD IS NOT NULL AND APPR_AWD != 'nan' AND APPR_AWD != ''
LEFT JOIN aims_lookup_auth AS auth_lookup ON 
    TRIM(AWD_AUTH) = auth_lookup.code AND AWD_AUTH IS NOT NULL AND AWD_AUTH != 'nan' AND AWD_AUTH != ''
LEFT JOIN aims_lookup_awd AS awd_lookup_secbd ON 
    TRIM(SECBD_RECMD_AWD) = awd_lookup_secbd.code AND SECBD_RECMD_AWD IS NOT NULL AND SECBD_RECMD_AWD != 'nan' AND SECBD_RECMD_AWD != ''
